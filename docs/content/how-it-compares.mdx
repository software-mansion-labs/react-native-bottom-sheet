...
- `BottomSheet` is inline. It renders in your screen’s React Native hierarchy,
  ...
  around it.
- `ModalBottomSheet` uses `BottomSheetProvider`’s React Native portal by
  default. Use this when you want modal behavior while keeping layering control
  in your React tree, so global UI such as toasts, menus, floating controls, or
  debug overlays can be arranged above or below the sheet by where you place
  them relative to the provider.
